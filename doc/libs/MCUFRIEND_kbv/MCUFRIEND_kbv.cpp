...
//#define SUPPORT_1289              //SSD1289,SSD1297 (ID=0x9797) +626 bytes,
// 0.03s #define SUPPORT_1580              //R61580 Untested
...
//#define SUPPORT_8347D             //HX8347-D, HX8347-G, HX8347-I, HX8367-A
//+520 bytes, 0.27s #define SUPPORT_8347A             //HX8347-A +500 bytes,
// 0.27s #define SUPPORT_8352A             //HX8352A +486 bytes, 0.27s #define
// SUPPORT_8352B             //HX8352B #define SUPPORT_8357D_GAMMA //monster 34
// byte #define SUPPORT_9163              // #define SUPPORT_9225 //ILI9225-B,
// ILI9225-G ID=0x9225, ID=0x9226, ID=0x6813 +380 bytes
...
MCUFRIEND_kbv::MCUFRIEND_kbv(int CS, int RS, int WR, int RD, int _RST)
...
static uint16_t color565_to_555(uint16_t color) {
    return (color & 0xFFC0) | ((color & 0x1F) << 1) |
           ((color & 0x01));  // lose Green LSB, extend Blue LSB
}
static uint16_t color555_to_565(uint16_t color) {
    return (color & 0xFFC0) | ((color & 0x0400) >> 5) |
           ((color & 0x3F) >> 1);  // extend Green LSB
}
...
//#define WriteCmdParam4(cmd, d1, d2, d3, d4) {uint8_t d[4];d[0] = d1, d[1] =
// d2, d[2] = d3, d[3] = d4;WriteCmdParamN(cmd, 4, d);}
void MCUFRIEND_kbv::pushCommand(uint16_t cmd, uint8_t *block, int8_t N) {
...
uint16_t MCUFRIEND_kbv::readID(void) {
    ...
    if (ret == 0x4747) return 0x8347;
    ret = readReg40(0xEF);  // ILI9327: [xx 02 04 93 27 FF]
    if (ret == 0x9327) return 0x9327;
    //#if defined(SUPPORT_1963) && USING_16BIT_BUS
    ret = readReg32(0xA1);  // SSD1963: [01 57 61 01]
    if (ret == 0x6101) return 0x1963;
    ...
    ret = readReg32(0xD7);
    if (ret ==
        0x8031)  // weird unknown from BangGood [xx 20 80 31] PrinceCharles
        return 0x8031;
    ret = readReg32(0xFE) >> 8;  // weird unknown from BangGood [04 20 53]
    if (ret == 0x2053) return 0x2053;
    ...
    //        return 0x1526;          //subsequent begin() enables Command
    //        Access
    ...
    parallel pushCommand(0xD9, &msb, 1); ret2 = readReg(0xD3); msb = 0x13;
...
// independent cursor and window registers.   S6D0154, ST7781 increments.
// ILI92320/5 do not.
int16_t MCUFRIEND_kbv::readGRAM(int16_t x, int16_t y, uint16_t *block,
                                int16_t w, int16_t h) {
    ...
    while (n > 0) {
        ...
        while (n) {
            if (_lcd_capable & READ_24BITS) {
                ...
                READ_16(ret);
                if (_lcd_capable & READ_LOWHIGH) ret = (ret >> 8) | (ret << 8);
                if (_lcd_capable & READ_BGR)
            ...
            *block++ = ret;
            n--;
            if (!(_lcd_capable & AUTO_READINC)) break;
        }
        if (++col >= w) {
            col = 0;
            if (++row >= h) row = 0;
...
void MCUFRIEND_kbv::setRotation(uint8_t r) {
    ...
    rotation =
        r &
        3;  // just perform the operation ourselves on the protected variables
    ...
    if (_lcd_capable & MIPI_DCS_REV1) {
        if (_lcd_ID == 0x6814) {  //.kbv my weird 0x9486 might be 68140
            ...
            goto common_MC;
                   ...
                   _lcd_ID == 0x1511) {
            if (val & 0x80) val |= 0x01;    // GS
            if ((val & 0x40)) val |= 0x02;  // SS
            ...
            //            val &= (_lcd_ID == 0x1963) ? ~0xC0 : ~0xD0; //MY=0,
            //            MX=0 with ML=0 for ILI9481
            goto common_MC;
        } else if (is8347) {
            _MC = 0x02, _MP = 0x06, _MW = 0x22, _SC = 0x02, _EC = 0x04,
            _SP = 0x06, _EP = 0x08;
    ...
    common_MC:
        _MC = 0x2A, _MP = 0x2B, _MW = 0x2C, _SC = 0x2A, _EC = 0x2A, _SP = 0x2B,
        _EP = 0x2B;
    common_BGR:
        WriteCmdParamN(is8347 ? 0x16 : 0x36, 1, &val);
        _lcd_madctl = val;
        //	    if (_lcd_ID	== 0x1963) WriteCmdParamN(0x13, 0, NULL);
        ////NORMAL mode
    }
    // cope with 9320 variants
        ...
        switch (_lcd_ID) {
            ...
            case 0x9225:
                ...
                SS_v = (val & 0x40) ? (1 << 8) : 0;
                WriteCmdData(0x01,
                             GS | SS_v | 0x001C);  // set Driver Output Control
                goto common_ORG;
#endif
            ...
            common_S6D:
                ...
                WriteCmdData(0x01,
                             GS | SS_v | ((_lcd_ID == 0x0139) ? 0x27 : 0x28));
                goto common_ORG;
#endif
            case 0x5420:
            case 0x7793:
            case 0x9326:
            case 0xB509:
                _MC = 0x200, _MP = 0x201, _MW = 0x202, _SC = 0x210, _EC = 0x211,
                _SP = 0x212, _EP = 0x213;
                GS = (val & 0x80) ? (1 << 15) : 0;
                NL = ((400 / 8) - 1) << 9;  // 400 rows
                if (_lcd_ID == 0x9326 ||
                    _lcd_ID == 0x5420) {  // NL and SCN are in diff position
                    if (GS)
                        GS |= (4 << 0);  // start SCN at row 32 PLEASE TEST
                                         // ILI9326, SPFD5420
                    NL >>= 1;
                }
                ...
                goto common_SS;
            default:
                _MC = 0x20, _MP = 0x21, _MW = 0x22, _SC = 0x50, _EC = 0x51,
                _SP = 0x52, _EP = 0x53;
                GS = (val & 0x80) ? (1 << 15) : 0;
...
#ifdef SUPPORT_8230
                if (_lcd_ID ==
                    0x8230) {  // UC8230 has strange BGR and READ_BGR behaviour
                    if (rotation == 1 || rotation == 2) {
                        val ^= 0x08;  // change BGR bit for LANDSCAPE and
                                      // PORTRAIT_REV
                    }
                }
#endif
                if (val & 0x08) ORG |= 0x1000;  // BGR
                _lcd_madctl = ORG | 0x0030;
                WriteCmdData(
                    0x03, _lcd_madctl);  // set GRAM write direction and BGR=1.
                break;
#ifdef SUPPORT_1289
            case 0x1289:
                _MC = 0x4E, _MP = 0x4F, _MW = 0x22, _SC = 0x44, _EC = 0x44,
                _SP = 0x45, _EP = 0x46;
                if (rotation & 1) val ^= 0xD0;  // exchange Landscape modes
                GS = (val & 0x80) ? (1 << 14)
                                  : 0;  // called TB (top-bottom), CAD=0
                SS_v = (val & 0x40) ? (1 << 9) : 0;  // called RL (right-left)
                ORG = (val & 0x20) ? (1 << 3) : 0;   // called AM
                _lcd_drivOut =
                    GS | SS_v | (REV << 13) | 0x013F;  // REV=0, BGR=0, MUX=319
                if (val & 0x08) _lcd_drivOut |= 0x0800;  // BGR
...
void MCUFRIEND_kbv::setAddrWindow(int16_t x, int16_t y, int16_t x1,
                                  int16_t y1) {
    if (_lcd_capable & MIPI_DCS_REV1) {
        WriteCmdParam4(_SC, x >> 8, x, x1 >> 8,
                       x1);  // Start column instead of _MC
...
void MCUFRIEND_kbv::fillRect(int16_t x, int16_t y, int16_t w, int16_t h,
                             uint16_t color) {
    int16_t end;
#if defined(SUPPORT_9488_555)
...
static void pushColors_any(uint16_t cmd, uint8_t *block, int16_t n, bool first,
                           uint8_t flags) {
    uint16_t color;
    uint8_t h, l;
...
void MCUFRIEND_kbv::vertScroll(int16_t top, int16_t scrollines,
                               int16_t offset) {
#if defined(OFFSET_9327)
    if (_lcd_ID == 0x9327) {
    ...
    if (_lcd_capable & MIPI_DCS_REV1) {
        ...
        //        if (offset == 0 && rotation > 1) vsp = top + scrollines;
        //        //make non-valid
        d[0] = vsp >> 8;  // VSP
        d[1] = vsp;
    ...
    switch (_lcd_ID) {
        ...
        case 0x0139:
            WriteCmdData(
                0x07, 0x0213 | (_lcd_rev << 2));  // VLE1=1, GON=1, REV=x, D=3
        ...
        case 0xB509:
            ...
            break;
        default:
            // 0x6809, 0x9320, 0x9325, 0x9335, 0xB505 can only scroll whole
            // screen
    ...
    if (_lcd_capable & MIPI_DCS_REV1) {
        if (is8347) {
            ...
            if (_lcd_ID == 0x8347 || _lcd_ID == 0x5252)  // HX8347-A, HX5352-A
                ...
                val = _lcd_rev
                          ? 8
                          : 10;  // HX8347-D, G, I: SCROLLON=bit3, INVON=bit1
...
*/
        ...
        case 0x0139:
            ...
            static const uint16_t S6D0139_regValues[] PROGMEM = {
                ...
                0x0011,
                0x1a00,  // Power Control 2
                0x0014,
                0x2020,  // Power Control 4
                0x0010,
                0x0900,  // Power Control 1
                0x0013,
                0x0040,  // Power Control 3
                0x0013,
                0x0060,  // Power Control 3
                0x0013,
                0x0070,  // Power Control 3
                0x0011,
                0x1a04,  // Power Control 2
                0x0010,
                0x2f00,  // Power Control 1
                0x0001,
                0x0127,  // Driver Control: SM=0, GS=0, SS=1, 240x320
                0x0002,
                0x0100,  // LCD Control:  (.kbv was 0700) FLD=0, BC= 0, EOR=1
                0x0003,
                0x1030,  // Entry Mode:    TR1=0, DFM=0, BGR=1, I_D=3
                0x0007,
                0x0000,  // Display Control: everything off
                0x0008,
                0x0303,  // Blank Period:  FP=3, BP=3
                0x0009,
                0x0000,  // f.k.
                0x000b,
                0x0000,  // Frame Control:
                0x000c,
                0x0000,  // Interface Control: system i/f
                0x0040,
                0x0000,  // Scan Line
                0x0041,
                0x0000,  // Vertical Scroll Control
                0x0007,
                0x0014,  // Display Control: VLE1=0, SPT=0, GON=1, REV=1, D=0
                         // (halt)
                0x0007,
                0x0016,  // Display Control: VLE1=0, SPT=0, GON=1, REV=1, D=2
                         // (blank)
                0x0007,
                0x0017,  // Display Control: VLE1=0, SPT=0, GON=1, REV=1, D=3
                         // (normal)
                //            0x0007, 0x0217,     //Display Control: VLE1=1,
                //            SPT=0, GON=1, REV=1, D=3
            };
        ...
        case 0x0154:
            ...
            static const uint16_t S6D0154_regValues[] PROGMEM = {
                0x0011, 0x001A, 0x0012, 0x3121,  // BT=3, DC1=1, DC2=2,
                                                 // DC3=1
                0x0013, 0x006C,                  // GVD=108
                0x0014, 0x4249,                  // VCM=66, VML=73

                0x0010, 0x0800,                    // SAP=8
                TFTLCD_DELAY, 10, 0x0011, 0x011A,  // APON=0, PON=1, AON=0,
                                                   // VCI1_EN=1, VC=10
                TFTLCD_DELAY, 10, 0x0011, 0x031A,  // APON=0, PON=3, AON=0,
                                                   // VCI1_EN=1, VC=10
                TFTLCD_DELAY, 10, 0x0011, 0x071A,  // APON=0, PON=7, AON=0,
                                                   // VCI1_EN=1, VC=10
                TFTLCD_DELAY, 10, 0x0011, 0x0F1A,  // APON=0, PON=15, AON=0,
                                                   // VCI1_EN=1, VC=10
                TFTLCD_DELAY, 10, 0x0011, 0x0F3A,  // APON=0, PON=15, AON=1,
                                                   // VCI1_EN=1, VC=10
                TFTLCD_DELAY, 30,

                0x0001, 0x0128, 0x0002, 0x0100, 0x0003, 0x1030, 0x0007, 0x1012,
                0x0008, 0x0303, 0x000B, 0x1100, 0x000C, 0x0000, 0x000F, 0x1801,
                0x0015, 0x0020,

                0x0050, 0x0101, 0x0051, 0x0603, 0x0052, 0x0408, 0x0053, 0x0000,
                0x0054, 0x0605, 0x0055, 0x0406, 0x0056, 0x0303, 0x0057, 0x0303,
                0x0058, 0x0010, 0x0059, 0x1000,

                0x0007, 0x0012,                    // GON=1, REV=0, D=2
                TFTLCD_DELAY, 40, 0x0007, 0x0013,  // GON=1, REV=0, D=3
        ...
        case 0x05A1:
            ...
            static const uint8_t S6D05A1_regValues_max[] PROGMEM = {
                0xF0, 2,    0x5A, 0x5A, 0xF1, 2,    0x5A, 0x5A, 0xF2, 19,
                0x3B, 0x33, 0x03, 0x0C, 0x08, 0x08, 0x08, 0x00, 0x08, 0x08,
                0x00, 0x00, 0x00, 0x00, 0x33, 0x0C, 0x08, 0x0C, 0x08, 0xF4,
                14,   0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04,
                0x70, 0x03, 0x04, 0x70, 0x03, 0xF5, 12,   0x00, 0x46, 0x70,
                0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x46, 0x70, 0xF6,
                8,    0x03, 0x00, 0x08, 0x03, 0x03, 0x00, 0x03, 0x00, 0xF7,
                5,    0x00, 0x80, 0x10, 0x02, 0x00, 0xF8, 2,    0x11, 0x00,
                0xF9, 1,    0x14, 0xFA, 16,   0x33, 0x07, 0x04, 0x1A, 0x18,
                0x1C, 0x24, 0x1D, 0x26, 0x28, 0x2F, 0x2E, 0x00, 0x00, 0x00,
                0x00, 0xFB, 16,   0x33, 0x03, 0x00, 0x2E, 0x2F, 0x28, 0x26,
                0x1D, 0x24, 0x1C, 0x18, 0x1A, 0x04, 0x00, 0x00, 0x00, 0xF9,
                1,    0x12, 0xFA, 16,   0x36, 0x07, 0x04, 0x1C, 0x1C, 0x23,
                0x28, 0x1C, 0x25, 0x26, 0x2E, 0x2B, 0x00, 0x00, 0x00, 0x00,
                0xFB, 16,   0x33, 0x06, 0x00, 0x2B, 0x2E, 0x26, 0x25, 0x1C,
                0x28, 0x23, 0x1C, 0x1C, 0x04, 0x00, 0x00, 0x00, 0xF9, 1,
                0x11, 0xFA, 16,   0x33, 0x07, 0x04, 0x30, 0x32, 0x34, 0x35,
                0x11, 0x1D, 0x20, 0x28, 0x20, 0x00, 0x00, 0x00, 0x00, 0xFB,
                16,   0x33, 0x03, 0x00, 0x20, 0x28, 0x20, 0x1D, 0x11, 0x35,
                0x34, 0x32, 0x30, 0x04, 0x00, 0x00, 0x00, 0x44, 2,    0x00,
                0x01,
            };
            table8_ads = S6D05A1_regValues_max,
            table_size = sizeof(S6D05A1_regValues_max);
            p16 = (int16_t *)&HEIGHT;
            *p16 = 480;
            p16 = (int16_t *)&WIDTH;
            *p16 = 320;
        ...
        case 0x9797:
            ...
            //        AUTO_READINC | READ_24BITS;
            // deliberately set READ_BGR to disable Software Scroll in
            // graphictest_kbv example
            _lcd_capable = 0 | XSA_XEA_16BIT | REV_SCREEN | AUTO_READINC |
                           READ_24BITS | READ_BGR;
            _lcd_ID = 0x1289;
            goto common_1289;
        case 0x1289:
            _lcd_capable = 0 | XSA_XEA_16BIT | REV_SCREEN | AUTO_READINC;
        common_1289:
            // came from MikroElektronika library
        ...
        case 0x1511:                                      // Unknown from Levy
            ...
            table8_ads = R61511_regValues,
            table_size = sizeof(R61511_regValues);
            p16 = (int16_t *)&HEIGHT;
            *p16 = 480;
            p16 = (int16_t *)&WIDTH;
            *p16 = 320;
        ...
        case 0x1520:
            ...
            table8_ads = R61520_regValues,
            table_size = sizeof(R61520_regValues);
            break;
        ...
        case 0x1526:
            ...
            table8_ads = R61526_regValues,
            table_size = sizeof(R61526_regValues);
            break;

#ifdef SUPPORT_1580
        case 0x1580:
            _lcd_capable = 0 | REV_SCREEN | READ_BGR | INVERT_GS |
                           READ_NODUMMY;  // thanks vanhan123
            static const uint16_t R61580_regValues[] PROGMEM = {
                ...
                //            0x0011, 0x17B0,          //DC1=7, DC0=3, VC=0 ?b12
                //            ?b7 vanhan123
                0x0012,
                0x01BF,  // VCMR=1, PSON=1, PON=1, VRH=15
                //            0x0012, 0x013A,          //VCMR=1, PSON=1, PON=1,
                //            VRH=10 vanhan123
                0x0013,
                0x1300,  // VDV=19
                TFTLCD_DELAY,
        ...
        case 0x1963:
            _lcd_capable = AUTO_READINC | MIPI_DCS_REV1 | READ_NODUMMY |
                           INVERT_SS | INVERT_RGB;
#if USING_16BIT_BUS
...
#define SSD1963_PIXDATA 0x00
            ...
            static const uint8_t SSD1963_NHD_70_regValues[] PROGMEM = {
                ...
                TFTLCD_DELAY8, 10, (0xE0), 1, 0x03,  // Lock PLL
                0x01, 0,                             // Soft Reset
                TFTLCD_DELAY8, 120, (0xB0), 7, 0x08, 0x80, 0x03, 0x1F, 0x01,
                0xDF, 0x00,                  // LCD SPECIFICATION
                (0xF0), 1, SSD1963_PIXDATA,  // was 00 pixel data interface
                //            (0x3A), 1, 0x60,        // SET R G B format = 6 6
                //            6
                (0xE6), 3, 0x0F, 0xFF,
            ...
            static const uint8_t SSD1963_800_regValues[] PROGMEM = {
                ...
                0xDF,
                0x00,  // LCD SPECIFICATION
                //            (0xB0), 7, 0x24, 0x00, 0x03, 0x1F, 0x01, 0xDF,
                //            0x2D,        //LCD SPECIFICATION
                (0xB4),
                8,
                0x03,
                0xA0,
                ...
                (0xF0),
                1,
                SSD1963_PIXDATA,  // pixel data interface
                TFTLCD_DELAY8,
                1,
                0x28,
                0,  // Display Off
            ...
            //        table8_ads = SSD1963_480_regValues, table_size =
            //        sizeof(SSD1963_480_regValues);
            table8_ads = SSD1963_800_regValues,
            table_size = sizeof(SSD1963_800_regValues);
            //        table8_ads = SSD1963_NHD_50_regValues, table_size =
            //        sizeof(SSD1963_NHD_50_regValues); table8_ads =
            //        SSD1963_NHD_70_regValues, table_size =
            //        sizeof(SSD1963_NHD_70_regValues); table8_ads =
            //        SSD1963_800NEW_regValues, table_size =
            //        sizeof(SSD1963_800NEW_regValues); table8_ads =
            //        SSD1963_800ALT_regValues, table_size =
            //        sizeof(SSD1963_800ALT_regValues);
            p16 = (int16_t *)&HEIGHT;
            *p16 = 480;
            p16 = (int16_t *)&WIDTH;
            *p16 = 800;
        ...
        case 0x3229:
            _lcd_capable = 0 | AUTO_READINC | MIPI_DCS_REV1 | MV_AXIS |
                           INVERT_SS;  // | READ_24BITS;
            static const uint8_t UNK3229_regValues[] PROGMEM = {
                //----------------Star Initial Sequence-------//
                //            0x11, 0, // exit sleep
                0x06,
                0,  // enter extern command [cmd=0x0607]
                0x07,
                0,
                0xb1,
                2,
                ...
                0x00,  // [01 10 00]
                /*
                            0xe0, 1, 0x05,            //I don't believe these
                   registers 0xe1, 1, 0x32,            //safer to use power-on
                   defaults 0xe2, 1, 0x77, 0xe3, 1, 0x77, 0xe4, 1, 0x7f, 0xe5,
                   1, 0xfa, 0xe6, 1, 0x00, 0xe7, 1, 0x74, 0xe8, 1, 0x27, 0xe9,
                   1, 0x10, 0xea, 1, 0xc0, 0xeb, 1, 0x25,
                */
                0xfa,
                0,  // exit extern command [cmd=0xFAFB]
                0xfb,
                0,
            };
            table8_ads = UNK3229_regValues,
            table_size = sizeof(UNK3229_regValues);  //
            break;

#ifdef SUPPORT_4532
        // Support for LG Electronics LGDP4532 (also 4531 i guess) by Leodino
        // v1.0 2-Nov-2016 based on data by waveshare and the datasheet of LG
        // Electronics My approach to get it working: the parameters by
        // waveshare did no make it function allright I started with remming
        // lines to see if this helped. Basically the stuff in range 41-93 gives
        // problems. The other lines that are REMmed give no problems, but it
        // seems default values are OK as well.
        case 0x4532:                        // thanks Leodino
            ...
            static const uint16_t LGDP4532_regValues[] PROGMEM = {
                ...
                0x0010,
                0x0628,  // Power control 1 SAP[2:0] BT[3:0] AP[2:0] DK DSTB SLP
                ...
                // 0x0015,0x0050, //Regulator control RSET RI[2:0] RV[2:0]
                // RCONT[2:0]
                0x0012, 0x0016,  // Power control 3 PON VRH[3:0]
                TFTLCD_DELAY, 50, 0x0010,
                0x5660,  // Power control 1 SAP[2:0] BT[3:0] AP[2:0] DK DSTB SLP
                TFTLCD_DELAY, 50,
                ...
                // 0x0007,0x0202, //Display Control 1 PTDE[1:0] BASEE GON DTE
                // COL D[1:0]
                TFTLCD_DELAY, 50,
                // 0x0008,0x0202, //Display Control 2 FP[3:0] BP[3:0] front and
                // back porch (blank period at begin and end..) 0x000A,0x0000,
                // //Test Register 1 (RA0h) Gamma adjustment
                0x0030, 0x0000, 0x0031, 0x0402, 0x0032, 0x0106, 0x0033, 0x0700,
                0x0034, 0x0104, 0x0035, 0x0301, 0x0036, 0x0707, 0x0037, 0x0305,
                0x0038, 0x0208, 0x0039, 0x0F0B, TFTLCD_DELAY, 50,
                // some of this stuff in range 41-93 really throws things
                // off.... 0x0041,0x0002, 0x0060,0x2700, //Driver Output Control
                // (R60h) 0x0061,0x0001, //Base Image Display Control (R61h)
                // 0x0090,0x0119,   //Panel Interface Control 1 (R90h) DIVI[1:0]
                // RTNI[4:0] 0x0092,0x010A,  //Panel Interface Control 2 (R92h)
                // NOWI[2:0] EQI2[1:0] EQI1[1:0] 0x0093,0x0004, //Panel
                // Interface Control 3 (R93h) MCPI[2:0] 0x00A0,0x0100, //Test
                // Register 1 (RA0h)
                TFTLCD_DELAY, 50, 0x0007,
                0x0133,  // Display Control 1 PTDE[1:0] BASEE GON DTE COL D[1:0]
                TFTLCD_DELAY, 50,
        ...
        case 0x4535:
            ...
            static const uint16_t LGDP4535_regValues[] PROGMEM = {
                ...
                40,
                0x0012,
                0x0012,  // set the back porch and
                         // front porch
                TFTLCD_DELAY,
                40,
                0x0010,
                0x3420,  // set non-display area
        ...
        case 0x6814:
            ...
            table8_ads = RM68140_regValues_max,
            table_size = sizeof(RM68140_regValues_max);
            p16 = (int16_t *)&HEIGHT;
            *p16 = 480;
            p16 = (int16_t *)&WIDTH;
            *p16 = 320;
        ...
        case 0x7735:  //
            ...
            static const uint8_t PROGMEM table7735S[] = {
                ...
                0xB3, 6, 0x01, 0x2C, 0x2D, 0x01, 0x2C,
                0x2D,           // [05 3C 3C 05 3C 3C]
        ...
        case 0x7783:
            ...
            static const uint16_t ST7781_regValues_CPT24[] PROGMEM = {
                ...
                100,
                0x0010,
                0x12B0,  // Power Control 1 SAP=1, BT=2, APE=1, AP=3
                TFTLCD_DELAY,
                50,
                0x0011,
                0x0007,  // Power Control 2 VC=7
                ...
                50,
                0x0012,
                0x008C,  // Power Control 3 VCIRE=1, VRH=12
                0x0013,
                0x1700,  // Power Control 4 VDV=23
                0x0029,
                0x0020,  // NVM read data 2 VCM=32
                TFTLCD_DELAY,
                50,
                0x0030,
                0x0000,  // Gamma Control 1 App Note CPT 2.4
                0x0031,
                0x0106,  // Gamma Control 2
                0x0032,
                0x0101,  // Gamma Control 3
                0x0035,
                0x0106,  // Gamma Control 4
                0x0036,
                0x0203,  // Gamma Control 5
                0x0037,
                0x0000,  // Gamma Control 6
                0x0038,
                0x0707,  // Gamma Control 7
                0x0039,
                0x0204,  // Gamma Control 8
                0x003C,
                0x0106,  // Gamma Control 9
                0x003D,
                0x0103,  // Gamma Control 10
                0x0060,
                0xA700,  // Driver Output Control (R60h) .kbv was 0xa700
                0x0061,
                0x0001,  // Driver Output Control (R61h)
                0x0090,
                0X0030,  // Panel Interface Control 1 (R90h)
            ...
            init_table16(ST7781_regValues_CPT24,
                         sizeof(ST7781_regValues_CPT24));
        ...
        case 0x7789:
            ...
            static const uint8_t ST7789_regValues_arcain6[] PROGMEM = {
                (0xB2), 5, 0x0C, 0x0C, 0x00, 0x33, 0x33,  // PORCTRK: Porch
                                                          ...
                                                          // anyway
                (0xB7), 1, 0x35,        // GCTRL: Gate Control [35]
                (0xBB), 1, 0x35,        // VCOMS: VCOM setting VCOM=??? [20]
                                        // VCOM=0.9
                ...
                0x4B, 0x08, 0x16, 0x14, 0x19, 0x20,  // PVGAMCTRL:
                                                     // Positive Voltage
                                                     // Gamma control
                (0xE1), 14, 0xD0, 0x00, 0x06, 0x09, 0x0B, 0x29, 0x36, 0x54,
                0x4B, 0x0D, 0x16, 0x14, 0x21, 0x20,  // NVGAMCTRL:
                                                     // Negative Voltage
                                                     // Gamma control
            };
            table8_ads = ST7789_regValues,
            table_size = sizeof(ST7789_regValues);  //
            break;

        case 0x8031:  // Unknown BangGood thanks PrinceCharles
            _lcd_capable = AUTO_READINC | MIPI_DCS_REV1 | MV_AXIS |
                           READ_24BITS | REV_SCREEN;
            static const uint8_t FK8031_regValues[] PROGMEM = {
                // 0xF2:8.2 = SM, 0xF2:8.0 = REV. invertDisplay(), vertScroll()
                // do not work
                0xF2, 11,   0x16, 0x16, 0x03, 0x08, 0x08,
                0x08, 0x08, 0x10, 0x04, 0x16, 0x16,  // f.k. 0xF2:8.2 SM=1
                0xFD, 3,    0x11, 0x02, 0x35,        // f.k 0xFD:1.1 creates
                                                     // contiguous scan lins
            };
            table8_ads = FK8031_regValues,
            table_size = sizeof(FK8031_regValues);
            break;
        ...
        case 0x5252:  // HX8352-A
            ...
            static const uint8_t HX8352A_regValues[] PROGMEM = {
                ...
                // Gamma  Setting
                0x3E, 12, 0xB0, 0x03, 0x10, 0x56, 0x13, 0x46, 0x23, 0x76, 0x00,
                0x5E, 0x4F, 0x40,
                // Power Voltage Setting
                0x17, 1, 0x91,  // OSC   1: RADJ=9, OSC_EN=1 [F0]
                0x2B, 1, 0xF9,  // Cycle 1: N_DC=F9 [BE]
                TFTLCD_DELAY8, 10, 0x1B, 1,
                0x14,           // Power 3: BT=1, ??=1, AP=0 [42]
                0x1A, 1, 0x11,  // Power 2: VC3=1, VC1=1 [05]
                0x1C, 1, 0x06,  // Power 4: VRH=6 [0D]
                0x1F, 1, 0x42,  // VCOM   : VCM=42 [55]
                TFTLCD_DELAY8, 20, 0x19, 1,
                0x0A,           // Power 1: DK=1, VL_TR1=1 [09]
                0x19, 1, 0x1A,  // Power 1: PON=1, DK=1, VL_TR1=1 [09]
                TFTLCD_DELAY8, 40, 0x19, 1,
                0x12,  // Power 1: PON=1, DK=1, STB=1 [09]
                TFTLCD_DELAY8, 40, 0x1E, 1,
                0x27,  // Power 6: VCOMG=1, VDV=7 [10]
                TFTLCD_DELAY8, 100,
                // Display ON Setting
                ...
                TFTLCD_DELAY8, 40, 0x24, 1, 0x3C,  // Display 2: GON=1 D=3 [A0]
                0x16, 1, 0x1C,  // Memaccess: GS=1, BGR=1, SS=1
                0x01, 1, 0x06,  // Disp Mode: INVON=1, NORON=1 [02]
                0x55, 1, 0x06,  // SM_PANEL=0, SS_PANEL=0, GS_PANEL=1,
                                // REV_PANEL=1, BGR_PANEL=0
            };
        ...
        case 0x0065:  // HX8352-B
            ...
            static const uint8_t HX8352B_regValues[] PROGMEM = {
                ...
                // Power on sequence
                0x19, 1, 0x01,                     // OSCEN=1
                TFTLCD_DELAY8, 5, 0x1F, 1, 0x8C,   // GASEN=1, DK=1, XDK=1
                0x1F, 1, 0x84,                     // GASEN=1, XDK=1
                TFTLCD_DELAY8, 10, 0x1F, 1, 0x94,  // GASEN=1, PON=1, XDK=1
                TFTLCD_DELAY8, 10, 0x1F, 1,
                0xD4,  // GASEN=1, VCOMG=1, PON=1, XDK=1
                TFTLCD_DELAY8, 5,
                // Gamma Setting
                0x40, 13, 0x00, 0x2B, 0x29, 0x3E, 0x3D, 0x3F, 0x24, 0x74, 0x08,
                0x06, 0x07, 0x0D, 0x17, 0x50, 13, 0x00, 0x02, 0x01, 0x16, 0x14,
                0x3F, 0x0B, 0x5B, 0x08, 0x12, 0x18, 0x19, 0x17, 0x5D, 1,
                0xFF,  //

                0x16, 1, 0x08,                     // MemoryAccess BGR=1
                0x28, 1, 0x20,                     // GON=1
                TFTLCD_DELAY8, 40, 0x28, 1, 0x38,  // GON=1, DTE=1, D=2
                TFTLCD_DELAY8, 40, 0x28, 1, 0x3C,  // GON=1, DTE=1, D=3
        ...
        case 0x8347:
            ...
            static const uint8_t HX8347A_CMO32_regValues[] PROGMEM = {
                ...
                (0x46), 12, 0xA4, 0x53, 0x00, 0x44, 0x04, 0x67, 0x33, 0x77,
                0x12, 0x4C, 0x46, 0x44,
                // Display Setting
                (0x01), 1, 0x06,  // IDMON=0, INVON=1, NORON=1, PTLON=0
                (0x16), 1, 0x48,  // MY=0, MX=0, MV=0, ML=1, BGR=0, TEON=0
                (0x23), 3, 0x95, 0x95,
                0xFF,  // N_DC=1001 0101, PI_DC=1001 0101, I_DC=1111 1111

                (0x27), 4, 0x02, 0x02, 0x02,
                0x02,                   // N_BP=2, N_FP=2, PI_BP=2, PI_FP=2
                (0x2C), 2, 0x02, 0x02,  // I_BP=2, I_FP=2

                (0x3a), 4, 0x01, 0x01, 0xF0,
                0x00,  // N_RTN=0, N_NW=1, P_RTN=0, P_NW=1, I_RTN=15, I_NW=0,
                       // DIV=0
                TFTLCD_DELAY8, 5, (0x35), 2, 0x38, 0x78,  // EQS=38h, EQP=78h
                (0x3E), 1, 0x38,                          // SON=38h
                (0x40), 2, 0x0F, 0xF0,                    // GDON=0Fh, GDOFF
                // Power Supply Setting
                (0x19), 1, 0x49,  // CADJ=0100, CUADJ=100, OSD_EN=1 ,60Hz
                (0x93), 1, 0x0F,  // RADJ=1111, 100%
                TFTLCD_DELAY8, 5, (0x20), 1, 0x40,  // BT=0100
                (0x1D), 3, 0x07, 0x00, 0x04,        // VC1=7, VC3=0, VRH=??
                // VCOM SETTING for 3.2"
                (0x44), 2, 0x4D, 0x11,               // VCM=100 1101, VDV=1 0001
                TFTLCD_DELAY8, 10, (0x1C), 1, 0x04,  // AP=100
                TFTLCD_DELAY8, 20, (0x1B), 1,
                0x18,  // GASENB=0, PON=0, DK=1, XDK=0, VLCD_TRI=0, STB=0
                TFTLCD_DELAY8, 40, (0x1B), 1,
                0x10,  // GASENB=0, PON=1, DK=0, XDK=0, VLCD_TRI=0, STB=0
                TFTLCD_DELAY8, 40, (0x43), 1, 0x80,  // set VCOMG=1
                TFTLCD_DELAY8, 100,
                // Display ON Setting
                (0x90), 1, 0x7F,                     // SAP=0111 1111
                (0x26), 1, 0x04,                     // GON=0, DTE=0, D=01
                TFTLCD_DELAY8, 40, (0x26), 1, 0x24,  // GON=1, DTE=0, D=01
                (0x26), 1, 0x2C,                     // GON=1, DTE=0, D=11
                TFTLCD_DELAY8, 40, (0x26), 1, 0x3C,  // GON=1, DTE=1, D=11
            ...
            static const uint8_t HX8347A_CMO24_regValues[] PROGMEM = {
                ...
                (0x46), 12, 0x94, 0x41, 0x00, 0x33, 0x23, 0x45, 0x44, 0x77,
                0x12, 0xCC, 0x46, 0x82,
                // Display Setting
                (0x01), 1, 0x06,  // IDMON=0, INVON=1, NORON=1, PTLON=0
                (0x16), 1, 0x48,  // MY=0, MX=0, MV=0, ML=1, BGR=0, TEON=0
            ...
            static const uint8_t HX8347A_ITDB02_regValues[] PROGMEM = {
                //  VENDOR Gamma ITDB02 same as CMO32.   Delays are shorter than
                //  AN01
                (0x46), 12, 0xA4, 0x53, 0x00, 0x44, 0x04, 0x67, 0x33, 0x77,
                0x12, 0x4C, 0x46, 0x44,
                // Display Setting
                (0x01), 1, 0x06,  // IDMON=0, INVON=1, NORON=1, PTLON=0
                (0x16), 1,
                0xC8,  // MY=0, MX=0, MV=0, ML=1, BGR=0, TEON=0 .itead
                (0x23), 3, 0x95, 0x95,
                0xFF,  // N_DC=1001 0101, PI_DC=1001 0101, I_DC=1111 1111
                ...
                TFTLCD_DELAY8, 5, (0x1C), 1, 0x04,  // AP=100
                TFTLCD_DELAY8, 5, (0x1B), 1,
                0x18,  // GASENB=0, PON=0, DK=1, XDK=0, VLCD_TRI=0, STB=0
                TFTLCD_DELAY8, 5, (0x1B), 1,
                0x10,  // GASENB=0, PON=1, DK=0, XDK=0, VLCD_TRI=0, STB=0
                TFTLCD_DELAY8, 5, (0x43), 1, 0x80,  // set VCOMG=1
                TFTLCD_DELAY8, 5,
                // Display ON Setting
                (0x90), 1, 0x7F,                    // SAP=0111 1111
                (0x26), 1, 0x04,                    // GON=0, DTE=0, D=01
                TFTLCD_DELAY8, 5, (0x26), 1, 0x24,  // GON=1, DTE=0, D=01
                (0x26), 1, 0x2C,                    // GON=1, DTE=0, D=11
                TFTLCD_DELAY8, 5, (0x26), 1, 0x3C,  // GON=1, DTE=1, D=11
                // INTERNAL REGISTER SETTING for ITDB02
                (0x57), 1, 0x02,  // TEST_Mode=1: into TEST mode
                (0x95), 1, 0x01,  // SET DISPLAY CLOCK AND PUMPING CLOCK TO
                                  // SYNCHRONIZE .itead
                (0x57), 1, 0x00,  // TEST_Mode=0: exit TEST mode
            };
            static const uint8_t HX8347A_NHD_regValues[] PROGMEM = {
                // Gamma Setting NHD
                (0x46), 12, 0x94, 0x41, 0x00, 0x33, 0x23, 0x45, 0x44, 0x77,
                0x12, 0xCC, 0x46, 0x82, (0x01), 1, 0x06,  // Display Mode [06]
                (0x16), 1, 0xC8,  // MADCTL [00] MY=1, MX=1, BGR=1
                //            (0x70), 1, 0x05,    //Panel [06] 16-bit
                (0x23), 3, 0x95, 0x95, 0xFF,  // Cycle Control 1-3 [95 95 FF]
                (0x27), 4, 0x02, 0x02, 0x02,
                0x02,                   // Display Control 2-5 [02 02 02 02]
                (0x2C), 2, 0x02, 0x02,  // Display Control 6-7 [02 02]
                (0x3A), 4, 0x01, 0x01, 0xF0,
                0x00,  // Cycle Control 1-4 [01 01 F0 00]
                TFTLCD_DELAY8, 80, (0x35), 2, 0x38,
                0x78,  // Display Control 9-10 [09 09] EQS=56, EQP=120
                ...
                TFTLCD_DELAY8, 80, (0x20), 1, 0x40,  // Power Control 6 [40]
                (0x1D), 3, 0x07, 0x00,
                0x04,  // Power Control 3-5 [04 00 06] VC=7
                (0x44), 2, 0x3C,
                0x12,  // VCOM Control 2-3 [5A 11] VCM=60, VDV=18
                TFTLCD_DELAY8, 80, (0x1C), 1, 0x04,  // Power Control 2 [04]
                TFTLCD_DELAY8, 80, (0x43), 1, 0x80,  // VCOM Control 1 [80]
                TFTLCD_DELAY8, 80, (0x1B), 1,
                0x08,  // Power Control 1 [00] DK=1
                TFTLCD_DELAY8, 80, (0x1B), 1,
                0x10,  // Power Control 1 [00] PON=1
                TFTLCD_DELAY8, 80, (0x90), 1, 0x7F,  // Display Control 8 [0A]
                (0x26), 1, 0x04,  // Display Control 1 [A0] D=1
                TFTLCD_DELAY8, 80, (0x26), 1,
                0x24,             // Display Control 1 [A0] GON=1, D=1
                (0x26), 1, 0x2C,  // Display Control 1 [A0] GON=1, D=3
                TFTLCD_DELAY8, 80, (0x26), 1,
                0x3C,             // Display Control 1 [A0] GON=1, DTE=1, D=3
                (0x57), 1, 0x02,  //?
                (0x55), 1, 0x00,  //?
                (0x57), 1, 0x00,  //?
            };
            // Atmel ASF code uses VCOM2-3: 0x38, 0x12. 50ms delays and no TEST
            // mode changes.
            init_table(HX8347A_NHD_regValues, sizeof(HX8347A_NHD_regValues));
            //        init_table(HX8347A_CMO32_regValues,
            //        sizeof(HX8347A_CMO32_regValues));
            //        init_table(HX8347A_CMO24_regValues,
            //        sizeof(HX8347A_CMO24_regValues));
            //        init_table(HX8347A_ITDB02_regValues,
            //        sizeof(HX8347A_ITDB02_regValues));
            //        init_table(HX8347G_2_regValues,
            //        sizeof(HX8347G_2_regValues));
            break;
#endif
        ...
        common_8357:
            ...
            table8_ads = HX8357C_regValues,
            table_size = sizeof(HX8357C_regValues);
            p16 = (int16_t *)&HEIGHT;
            *p16 = 480;
            p16 = (int16_t *)&WIDTH;
            *p16 = 320;
        ...
        case 0x0099:  // HX8357-D matches datasheet
            ...
            static const uint8_t HX8357_99_regValues[] PROGMEM = {
                ...
                // HX8357D_SETGAMMA [0B 0C 11 1D 25 37 43 4B 4E 47 41 39 35 31
                // 2E 21 1C 1D 1D 26 31 44 4E 56 44 3F 39 33 31 2E 28 1D E0 01]
                (0xE0),
                34,
                0x02,
                0x0A,
            ...
            table8_ads = HX8357_99_regValues,
            table_size = sizeof(HX8357_99_regValues);
            p16 = (int16_t *)&HEIGHT;
            *p16 = 480;
            p16 = (int16_t *)&WIDTH;
            *p16 = 320;
        ...
        case 0x8230:  // thanks Auzman
            ...
            static const uint16_t UC8230_regValues[] PROGMEM = {
                ...
                0x0060,
                0xa700,  // GS; gate scan: start position & drive line Q'ty
                0x0061, 0x0001,  // REV, NDL, VLE
                /*--------------------Gamma control------------------------*/
                0x0030, 0x0303, 0x0031, 0x0303, 0x0032, 0x0303, 0x0033, 0x0300,
                0x0034, 0x0003, 0x0035, 0x0303, 0x0036, 0x1400, 0x0037, 0x0303,
                0x0038, 0x0303, 0x0039, 0x0303, 0x003a, 0x0300, 0x003b, 0x0003,
                0x003c, 0x0303, 0x003d, 0x1400,
                //-----------------------------------------------------------//
                ...
                //-----------------------------------------------------------//
                0x0092, 0x0200, 0x0093, 0x0303, 0x0090,
                0x0010,                             // set clocks/Line
                0x0000, 0x0001, TFTLCD_DELAY, 200,  // Delay 200 ms
...
#define ILI9225_HORIZONTAL_WINDOW_ADDR1 \
    (0x36u)  // Horizontal Address END Position   HEA
#define ILI9225_HORIZONTAL_WINDOW_ADDR2 \
    (0x37u)  // Horizontal Address START Position HSA
#define ILI9225_VERTICAL_WINDOW_ADDR1 \
    (0x38u)  // Vertical Address END Position     VEA
#define ILI9225_VERTICAL_WINDOW_ADDR2 \
    (0x39u)                           // Vertical Address START Position   VSA
...
#define ILI9225C_INVON 0x21
        ...
        case 0x0001:
            _lcd_capable =
                0 | REV_SCREEN | INVERT_GS;  // no RGB bug. thanks Ivo_Deshev
            goto common_9320;
        case 0x5408:
            _lcd_capable = 0 | REV_SCREEN |
                           READ_BGR;  // Red 2.4" thanks jorgenv, Ardlab_Gent
            //        _lcd_capable = 0 | REV_SCREEN | READ_BGR | INVERT_GS;
            //        //Blue 2.8" might be different
            goto common_9320;
        case 0x1505:  // R61505 thanks Ravi_kanchan2004. R61505V, R61505W
                      // different
        case 0x9320:
        ...
        common_93x5:
            static const uint16_t ILI9325_regValues[] PROGMEM = {
                ...
                0x0084, 0x0000, 0x0085, 0x0000,
                //-------------- Panel Control -------------------//
                0x0090, 0x0010, 0x0092, 0x0000, 0x0007,
        ...
        case 0x9326:
            ...
            init_table16(ILI9326_CPT28_regValues,
                         sizeof(ILI9326_CPT28_regValues));
            p16 = (int16_t *)&HEIGHT;
            *p16 = 400;
            p16 = (int16_t *)&WIDTH;
            *p16 = 240;
        ...
        case 0x9327:
            ...
            static const uint8_t ILI9327_regValues[] PROGMEM = {
                ...
                //            0xD1, 3, 0x00, 0x71, 0x19,  //VCOM control [00 40
                //            0F] 0xD0, 3, 0x07, 0x01, 0x08,  //Power Setting
                //            [07 04 8C]
                0xC1, 4, 0x10, 0x10, 0x02,
                0x02,  // Display Timing [10 10 02 02]
                0xC0, 6, 0x00, 0x35, 0x00, 0x00, 0x01,
                0x02,  // Panel Drive [00 35 00 00 01 02 REV=0,GS=0,SS=0
                0xC5, 1, 0x04,        // Frame Rate [04]
                0xD2, 2, 0x01, 0x04,  // Power Setting [01 44]
                //            0xC8, 15, 0x04, 0x67, 0x35, 0x04, 0x08, 0x06,
                //            0x24, 0x01, 0x37, 0x40, 0x03, 0x10, 0x08, 0x80,
                //            0x00, 0xC8, 15, 0x00, 0x77, 0x77, 0x04, 0x04,
                //            0x00, 0x00, 0x00, 0x77, 0x00, 0x00, 0x08, 0x00,
                //            0x00, 0x00,
                0xCA, 1, 0x00,  // DGC LUT ???
                0xEA, 1, 0x80,  // 3-Gamma Function Enable
                //                     0xB0, 1, 0x03,      //Enable Protect
            };
            table8_ads = ILI9327_regValues,
            table_size = sizeof(ILI9327_regValues);
            p16 = (int16_t *)&HEIGHT;
            *p16 = 400;
            p16 = (int16_t *)&WIDTH;
            *p16 = 240;
        ...
        case 0x1602:
            _lcd_capable =
                AUTO_READINC | MIPI_DCS_REV1 | MV_AXIS;  // does not readGRAM
            static const uint8_t XX1602_regValues[] PROGMEM = {
                0xB8, 1, 0x01,  // GS [00]
                0xC0, 1, 0x0E,  //??Power [0A]
            };
            table8_ads = XX1602_regValues,
            table_size = sizeof(XX1602_regValues);
            break;

        case 0xE300:  // weird from BangGood
            _lcd_capable = AUTO_READINC | MIPI_DCS_REV1 | MV_AXIS |
                           READ_24BITS | INVERT_SS | INVERT_RGB;
            goto common_9329;
        case 0x2053:  // weird from BangGood
            _lcd_capable = AUTO_READINC | MIPI_DCS_REV1 | MV_AXIS |
                           READ_24BITS | REV_SCREEN | READ_BGR;
            goto common_9329;
        case 0xAC11:
            _lcd_capable = AUTO_READINC | MIPI_DCS_REV1 | MV_AXIS |
                           READ_24BITS | REV_SCREEN;  // thanks viliam
            goto common_9329;
        case 0x9302:
            _lcd_capable = AUTO_READINC | MIPI_DCS_REV1 | MV_AXIS;
            goto common_9329;
        ...
        case 0x9329:
            _lcd_capable =
                AUTO_READINC | MIPI_DCS_REV1 | MV_AXIS | INVERT_SS | REV_SCREEN;
        common_9329:
            static const uint8_t ILI9329_regValues[] PROGMEM = {
                //            0xF6, 3, 0x01, 0x01, 0x00,  //Interface Control
                //            needs EXTC=1 MX_EOR=1, TM=0, RIM=0
                //            0xB6, 3, 0x0A, 0x82, 0x27,  //Display Function [0A
                //            82 27]
                //            0xB7, 1, 0x06,      //Entry Mode Set [06]
                0x36, 1,
                0x00,  // Memory Access [00] pointless but stops an empty array
            };
            table8_ads = ILI9329_regValues,
            table_size = sizeof(ILI9329_regValues);
            break;

        case 0x9340:  // ILI9340 thanks Ravi_kanchan2004 and valmor_jr
            //_lcd_capable = AUTO_READINC | MIPI_DCS_REV1 | MV_AXIS |
            // READ_24BITS | REV_SCREEN; //Ravi
            _lcd_capable = AUTO_READINC | MIPI_DCS_REV1 | MV_AXIS |
                           READ_24BITS;  // my new shield
            goto common_9341;
        case 0x9341:
            _lcd_capable = AUTO_READINC | MIPI_DCS_REV1 | MV_AXIS | READ_24BITS;
        common_9341:
            static const uint8_t ILI9341_regValues_2_4[] PROGMEM = {
                ...
                //            0xB6, 2, 0x0A, 0xA2, 0x27, //Display Function [0A
                //            82 27 XX]    .kbv SS=1
                0xB4,
                1,
            ...
            static const uint8_t ILI9341_regValues_ada[] PROGMEM = {
                ...
                //                     0xF6, 3, 0x00, 0x01, 0x00,  //Interface
                //                     Control needs EXTC=1 TM=0, RIM=0
                //            0xF6, 3, 0x01, 0x01, 0x03,  //Interface Control
                //            needs EXTC=1 RM=1, RIM=1
                0xF6, 3, 0x09, 0x01,
                0x03,           // Interface Control needs EXTC=1 RM=0, RIM=1
                0xB0, 1, 0x40,  // RGB Signal [40] RCM=2
                0xB4, 1, 0x00,  // Inversion Control [02] .kbv NLA=1, NLB=1,
                                // NLC=1
            ...
            table8_ads = ILI9341_regValues_2_4,
            table_size = sizeof(ILI9341_regValues_2_4);  //
            break;
#if defined(SUPPORT_9342)
        case 0x9342:
            _lcd_capable = AUTO_READINC | MIPI_DCS_REV1 | MV_AXIS |
                           READ_24BITS | INVERT_GS | REV_SCREEN;
            static const uint8_t ILI9342_regValues_CPT24[] PROGMEM = {
                ...
                (0xC7), 1,    0xC3,              //[C0]
                (0xB8), 1,    0x0B,              //[07]
                (0xE0), 15,   0x0F, 0x33, 0x30, 0x0C, 0x0F, 0x08, 0x5D,
                0x66,   0x4A, 0x07, 0x13, 0x05, 0x1B, 0x0E, 0x08, (0xE1),
                15,     0x08, 0x0E, 0x11, 0x02, 0x0E, 0x02, 0x24, 0x33,
                0x37,   0x03, 0x0A, 0x09, 0x26, 0x33, 0x0F,
            };
            static const uint8_t ILI9342_regValues_Tianma23[] PROGMEM = {
                // Tianma 2.3"
                (0xB9), 3,      0xFF, 0x93,   0x42,   (0xC0), 2,      0x1D,
                0x0A,   (0xC1), 1,    0x01,   (0xC5), 2,      0x2C,   0x2C,
                (0xC7), 1,      0xC6, (0xB8), 1,      0x09,   (0xE0), 15,
                0x0F,   0x26,   0x21, 0x07,   0x0A,   0x03,   0x4E,   0x62,
                0x3E,   0x0B,   0x11, 0x00,   0x08,   0x02,   0x00,   (0xE1),
                15,     0x00,   0x19, 0x1E,   0x03,   0x0E,   0x03,   0x30,
                0x23,   0x41,   0x03, 0x0B,   0x07,   0x2F,   0x36,   0x0F,
            };
            static const uint8_t ILI9342_regValues_HSD23[] PROGMEM = {
                // HSD 2.3"
                (0xB9), 3,      0xFF, 0x93,   0x42,   (0xC0), 2,      0x1D,
                0x0A,   (0xC1), 1,    0x02,   (0xC5), 2,      0x2F,   0x27,
                (0xC7), 1,      0xA4, (0xB8), 1,      0x0B,   (0xE0), 15,
                0x0F,   0x24,   0x21, 0x0C,   0x0F,   0x06,   0x50,   0x75,
                0x3F,   0x07,   0x12, 0x05,   0x11,   0x0B,   0x08,   (0xE1),
                15,     0x08,   0x1D, 0x20,   0x02,   0x0E,   0x04,   0x31,
                0x24,   0x42,   0x03, 0x0B,   0x09,   0x30,   0x36,   0x0F,
            };
            table8_ads = ILI9342_regValues_CPT24,
            table_size = sizeof(ILI9342_regValues_CPT24);  //
            //        table8_ads = ILI9342_regValues_Tianma23, table_size =
            //        sizeof(ILI9342_regValues_Tianma23);   // table8_ads =
            //        ILI9342_regValues_HSD23, table_size =
            //        sizeof(ILI9342_regValues_HSD23);   //
            p16 = (int16_t *)&HEIGHT;
            *p16 = 240;
            p16 = (int16_t *)&WIDTH;
            *p16 = 320;
        ...
        common_9481:
            static const uint8_t ILI9481_regValues[] PROGMEM = {
                ...
                0xB3, 4, 0x02, 0x00, 0x00, 0x00,  // Frame Memory, interface
                                                  // [02 00 00 00]
                0xB4, 1, 0x00,                    // Frame mode [00]
                0xD0, 3, 0x07, 0x42, 0x18,        // Set Power [00 43 18] x1.00,
                                                  // x6, x3
                0xD1, 3, 0x00, 0x07, 0x10,        // Set VCOM  [00 00 00] x0.72,
                                                  // x1.02
                ...
                0xC0, 5, 0x12, 0x3B, 0x00, 0x02, 0x11,  // Panel Driving BGR
                                                        // for 1581 [10 3B 00
                                                        // 02 11]
                0xC1, 3, 0x10, 0x10, 0x88,  // Display Timing Normal [10 10
                                            // 88]
                0xC5, 1, 0x03,              // Frame Rate [03]
                0xC6, 1, 0x02,              // Interface Control [02]
                0xC8, 12, 0x00, 0x32, 0x36, 0x45, 0x06, 0x16, 0x37, 0x75, 0x77,
                0x54, 0x0C, 0x00, 0xCC, 1, 0x00,  // Panel
                                                  // Control [00]
            };
            ...
            static const uint8_t ILI9481_RGB_regValues[] PROGMEM = {
                ...
                0xC8, 12, 0x00, 0x14, 0x33, 0x10, 0x00, 0x16, 0x44, 0x36, 0x77,
                0x00, 0x0F, 0x00, 0xF3, 2, 0x40, 0x0A, 0xF0, 1, 0x08, 0xF6, 1,
                0x84, 0xF7, 1, 0x80, 0x0C, 2, 0x00, 0x55,  // RDCOLMOD
                0xB4, 1, 0x00,                             // SETDISPLAY
                //			0xB3, 4, 0x00, 0x01, 0x06, 0x01,
                ////SETGRAM
                // simple example
                0xB3, 4, 0x00, 0x01, 0x06, 0x30,  // jpegs example
            };
            table8_ads = ILI9481_regValues,
            table_size = sizeof(ILI9481_regValues);
            //        table8_ads = ILI9481_CPT29_regValues, table_size =
            //        sizeof(ILI9481_CPT29_regValues); table8_ads =
            //        ILI9481_PVI35_regValues, table_size =
            //        sizeof(ILI9481_PVI35_regValues); table8_ads =
            //        ILI9481_AUO317_regValues, table_size =
            //        sizeof(ILI9481_AUO317_regValues); table8_ads =
            //        ILI9481_CMO35_regValues, table_size =
            //        sizeof(ILI9481_CMO35_regValues); table8_ads =
            //        ILI9481_RGB_regValues, table_size =
            //        sizeof(ILI9481_RGB_regValues);
            p16 = (int16_t *)&HEIGHT;
            *p16 = 480;
            p16 = (int16_t *)&WIDTH;
            *p16 = 320;
        ...
        case 0x9486:
            _lcd_capable =
                AUTO_READINC | MIPI_DCS_REV1 | MV_AXIS;  // Red 3.5", Blue 3.5"
            //        _lcd_capable = AUTO_READINC | MIPI_DCS_REV1 | MV_AXIS |
            //        REV_SCREEN; //old Red 3.5"
            static const uint8_t ILI9486_regValues[] PROGMEM = {
                /*
                            0xF2, 9, 0x1C, 0xA3, 0x32, 0x02, 0xB2, 0x12, 0xFF,
                   0x12, 0x00,        //f.k 0xF1, 2, 0x36, 0xA4,        // 0xF8,
                   2, 0x21, 0x04,        // 0xF9, 2, 0x00, 0x08,        //
                */
                0xC0,
                2,
                0x0d,
...
#define GAMMA9486 4
            ...
            table8_ads = ILI9486_regValues,
            table_size = sizeof(ILI9486_regValues);
            p16 = (int16_t *)&HEIGHT;
            *p16 = 480;
            p16 = (int16_t *)&WIDTH;
            *p16 = 320;
        ...
        case 0x7796:
            _lcd_capable =
                AUTO_READINC | MIPI_DCS_REV1 | MV_AXIS;  // thanks to safari1
            static const uint8_t PROGMEM ST7796_regValues[] = {
                0xB7, 1,    0xC6,  // Entry Mode      [06]
                0xE8, 8,    0x40, 0x8A, 0x00,
                0x00, 0x29, 0x19, 0xA5, 0x33,  // Adj3 [40 8A 00 00 25 0A 38 33]
            };
            table8_ads = ST7796_regValues,
            table_size = sizeof(ST7796_regValues);
            p16 = (int16_t *)&HEIGHT;
            *p16 = 480;
            p16 = (int16_t *)&WIDTH;
            *p16 = 320;
        ...
        common_9488:
            static const uint8_t ILI9488_regValues_max[] PROGMEM = {
                ...
                0xC5, 4,    0x00, 0x22,
                0x80, 0x40,              // VCOM  Control 1 [00 40 00 40]
                ...
                0xB6, 3,    0x02, 0x02,
                0x3B,  // Display Function Control [02 02 3B] .kbv NL=480
                0xB7, 1,    0xC6,  // Entry Mode      [06]
                0x3A, 1,    0x55,  // Interlace Pixel Format [XX]
                0xF7, 4,    0xA9, 0x51,
                0x2C, 0x82,  // Adjustment Control 3 [A9 51 2C 82]
            };
            table8_ads = ILI9488_regValues_max,
            table_size = sizeof(ILI9488_regValues_max);
            p16 = (int16_t *)&HEIGHT;
            *p16 = 480;
            p16 = (int16_t *)&WIDTH;
            *p16 = 320;
        ...
        case 0xC505:  // R61505W
            ...
            static const uint16_t R61505V_regValues[] PROGMEM = {
                ...
                TFTLCD_DELAY, 10, 0x0060, 0x2700,  // NL
                0x0008, 0x0808,                    // FP & BP
                ...
                0x0039, 0x0020, 0x0090, 0x0015,  // DIVI & RTNI
                ...
                0x0003, 0x1030, 0x0009, 0x0001,  // ISC=1 [0000]
                0x000A, 0x0000,                  // [0000]
                //            0x000C, 0x0001,   //RIM=1 [0000]
                0x000D, 0x0000,  // [0000]
                0x000E, 0x0030,  // VEM=3 VCOM equalize [0000]
                0x0061, 0x0001, 0x006A, 0x0000, 0x0080, 0x0000, 0x0081, 0x0000,
                0x0082, 0x005F, 0x0092, 0x0100, 0x0093, 0x0701, TFTLCD_DELAY,
                80, 0x0007, 0x0100,  // BASEE=1--Display On
            };
        ...
        case 0x9806:
            ...
            table8_ads = ILI9806_regValues,
            table_size = sizeof(ILI9806_regValues);
            p16 = (int16_t *)&HEIGHT;
            *p16 = 480;
            p16 = (int16_t *)&WIDTH;
            *p16 = 854;
    ...
    if (table8_ads != NULL) {
        static const uint8_t reset_off[] PROGMEM = {
            0x01, 0,             // Soft Reset
            TFTLCD_DELAY8, 150,  // .kbv will power up with ONLY reset, sleep
                                 // out, display on
            0x28, 0,             // Display Off
